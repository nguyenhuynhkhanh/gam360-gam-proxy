...
def check_proxy_secret():
    ...
    # TEMPORARY (seed-welcome): the one-click admin page is opened in a browser,
    # which can't send the X-Proxy-Secret header. It does its own ?key= check
    # against PROXY_SECRET instead. Remove this exemption with the routes below.
    if request.path.startswith("/admin/seed-welcome"):
        return
    if not PROXY_SECRET:
...
# ===========================================================================
# TEMPORARY — one-click "welcome ad unit" seeder.
#
# Adds a small admin page with a button that creates a `welcome` (app-open) ad
# unit under every platform-root node, then is removed in the next deploy.
# Welcome ads are shown when the user opens the app, so they live at the
# platform root (one level under each `vtvprime_<env>` root), not under a
# content location.
#
# This whole block (the two routes + the helpers + the before_request
# exemption above) is intended to be deleted in the follow-up deployment.
# It is idempotent: GAM rejects a duplicate code under the same parent with
# ALREADY_EXISTS, which is reported as "skipped", so pressing the button more
# than once never creates a second unit.
# ===========================================================================

# Fullscreen interstitial sizes for app-open / welcome creatives:
# 320x480 portrait fullscreen, with a 300x250 medium-rectangle fallback.
_WELCOME_SIZES = [
    {"size": {"width": 320, "height": 480, "isAspectRatio": False}, "environmentType": "BROWSER"},
    {"size": {"width": 300, "height": 250, "isAspectRatio": False}, "environmentType": "BROWSER"},
]
_WELCOME_NAME = "welcome"
_WELCOME_CODE = "welcome"


def _seed_welcome_authorized():
    """The admin page can't send the secret header, so it passes ?key=.
    Allow when no secret is configured (local dev) or when key matches."""
    if not PROXY_SECRET:
        return True
    key = request.values.get("key", "")
    return key == PROXY_SECRET


def _discover_platform_roots():
    """Return the platform-root ad units: the nodes one level below each
    top-level `vtvprime_<env>` root (which itself sits under the network's
    effective root). Returns a list of {id, name, adUnitCode} dicts.

    No IDs are hard-coded — the hierarchy is walked from the network root so
    this works across whatever envs exist (dev/staging/prod)."""
    network = _network_service().getCurrentNetwork()
    effective_root = str(network["effectiveRootAdUnitId"])

    inventory_service = _inventory_service()
    page_size = 500
    offset = 0
    nodes = {}  # id -> {id, name, parentId, adUnitCode}
    ...
    while True:
        ...
        for ad_unit in results:
            uid = str(ad_unit["id"] if isinstance(ad_unit, dict) else ad_unit.id)
            name = (ad_unit["name"] if isinstance(ad_unit, dict) else getattr(ad_unit, "name", None)) or ""
            code = (ad_unit["adUnitCode"] if isinstance(ad_unit, dict) else getattr(ad_unit, "adUnitCode", None)) or ""
            parent_id = ad_unit["parentId"] if isinstance(ad_unit, dict) else getattr(ad_unit, "parentId", None)
            nodes[uid] = {
                "id": uid,
                "name": str(name),
    ...
    # L1 = top-level env roots (children of the network's effective root).
    l1_ids = {n["id"] for n in nodes.values() if n["parentId"] == effective_root}
    # L2 = platform roots (children of an L1 env root) — mob / ctv / web / tab.
    platform_roots = [n for n in nodes.values() if n["parentId"] in l1_ids]
    platform_roots.sort(key=lambda n: n["adUnitCode"] or n["id"])
    return platform_roots


@app.route("/admin/seed-welcome", methods=["GET"])
def seed_welcome_page():
    """TEMPORARY: serve the one-click welcome-ad seeder page."""
    return _SEED_WELCOME_HTML, 200, {"Content-Type": "text/html; charset=utf-8"}


@app.route("/admin/seed-welcome/targets", methods=["GET"])
def seed_welcome_targets():
    """TEMPORARY: preview which platform roots would get a `welcome` child."""
    if not _seed_welcome_authorized():
        return jsonify({"error": "UNAUTHORIZED", "message": "Invalid or missing key"}), 401
    try:
        roots = _discover_platform_roots()
        return jsonify({
            "code": _WELCOME_CODE,
            "name": _WELCOME_NAME,
            "sizes": [f'{s["size"]["width"]}x{s["size"]["height"]}' for s in _WELCOME_SIZES],
            "platformRoots": roots,
        }), 200
    except Exception as exc:
        logger.exception("seed-welcome target discovery failed")
        return jsonify({"error": "GAM_ERROR", "message": _sanitize_fault_message(exc)}), 500


@app.route("/admin/seed-welcome", methods=["POST"])
def seed_welcome_apply():
    """TEMPORARY: create a `welcome` ad unit under every platform root.
    Idempotent — units that already exist are reported as skipped."""
    if not _seed_welcome_authorized():
        return jsonify({"error": "UNAUTHORIZED", "message": "Invalid or missing key"}), 401
    ...
    try:
        ...
        logger.exception("seed-welcome discovery failed")
        return jsonify({"error": "GAM_ERROR", "message": _sanitize_fault_message(exc)}), 500

    if not roots:
        return jsonify({
            "error": "GAM_ERROR",
            "message": "No platform-root ad units found below the network root.",
        }), 404
    ...
    for root in roots:
        target = {
            "platformId": root["id"],
            "platformCode": root["adUnitCode"],
            "welcomeCode": (f'{root["adUnitCode"]}/{_WELCOME_CODE}' if root["adUnitCode"] else _WELCOME_CODE),
        }
        ad_unit = {
            "name": _WELCOME_NAME,
            "parentId": int(root["id"]),
            "adUnitCode": _WELCOME_CODE,
            "adUnitSizes": _WELCOME_SIZES,
        }
        try:
            result = inventory_service.createAdUnits([ad_unit])
            new = result[0]
            new_id = str(new["id"] if isinstance(new, dict) else new.id)
            logger.info("seed-welcome: created id=%s under platform=%s", new_id, root["id"])
            created.append({**target, "id": new_id})
        except errors.GoogleAdsServerFault as fault:
            if _is_already_exists(fault):
                logger.info("seed-welcome: already exists under platform=%s — skipped", root["id"])
                skipped.append({**target, "reason": "already exists"})
            else:
                logger.error("seed-welcome: create failed under platform=%s: %s", root["id"], fault)
                failed.append({**target, "reason": _sanitize_fault_message(fault)})
        except Exception as exc:
            logger.exception("seed-welcome: unexpected error under platform=%s", root["id"])
            failed.append({**target, "reason": _sanitize_fault_message(exc)})
...
_SEED_WELCOME_HTML = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Seed welcome ad units</title>
<style>
  ...
  h1 { font-size: 1.4rem; }
  .banner { background: #fff4e5; border: 1px solid #ffe0b2; border-left: 4px solid #f5a623;
            padding: 12px 16px; border-radius: 6px; margin: 16px 0; font-size: .9rem; }
  button { font-size: 1rem; padding: 10px 18px; border-radius: 6px; border: 0;
           cursor: pointer; margin-right: 8px; }
  #apply { background: #2563eb; color: #fff; }
  #apply:disabled { background: #9db8ef; cursor: default; }
  #preview { background: #eee; }
  table { border-collapse: collapse; width: 100%; margin-top: 16px; font-size: .9rem; }
  th, td { border: 1px solid #ddd; padding: 6px 10px; text-align: left; }
  th { background: #f5f5f5; }
  .out { margin-top: 20px; white-space: pre-wrap; font-family: ui-monospace, monospace;
         font-size: .85rem; background: #f7f7f7; padding: 12px; border-radius: 6px; }
  .ok { color: #15803d; } .warn { color: #b45309; } .err { color: #b91c1c; }
</style>
</head>
<body>
<h1>Welcome ad unit seeder</h1>
<div class="banner">
  <strong>Temporary tool.</strong> Creates a <code>welcome</code> (app-open) ad unit
  under every platform root. Safe to press more than once — existing units are skipped.
  This page is removed in the next deployment.
</div>
<p>Sizes: <code>320x480</code>, <code>300x250</code> (BROWSER). Name/code: <code>welcome</code>.</p>
<div>
  <button id="preview" type="button">Preview targets</button>
  <button id="apply" type="button">Create welcome ad units</button>
</div>
<div id="targets"></div>
...
<script>
  ...
  function show(el, text, cls) { el.hidden = false; el.textContent = text; el.className = "out " + (cls||""); }

  async function preview() {
    targetsEl.innerHTML = "Loading targets…";
    try {
      const r = await fetch("/admin/seed-welcome/targets" + qs);
      const d = await r.json();
      if (!r.ok) { targetsEl.innerHTML = ""; show(resultEl, JSON.stringify(d, null, 2), "err"); return; }
      const rows = (d.platformRoots||[]).map(p =>
        `<tr><td>${p.adUnitCode||"(no code)"}</td><td>${p.id}</td><td>${p.adUnitCode? p.adUnitCode+"/"+d.code : d.code}</td></tr>`).join("");
      targetsEl.innerHTML = (d.platformRoots||[]).length
        ? `<p>${d.platformRoots.length} platform root(s) found:</p>
           <table><tr><th>Platform</th><th>Parent ID</th><th>New welcome code</th></tr>${rows}</table>`
        : "<p class='warn'>No platform roots found.</p>";
    } catch (e) { targetsEl.innerHTML = ""; show(resultEl, String(e), "err"); }
  }

  async function apply() {
    const btn = document.getElementById("apply");
    btn.disabled = true; show(resultEl, "Creating…");
    try {
      const r = await fetch("/admin/seed-welcome" + qs, { method: "POST" });
      const d = await r.json();
      const cls = d.summary && d.summary.failed ? "warn" : "ok";
      show(resultEl, JSON.stringify(d, null, 2), r.ok ? cls : "err");
    } catch (e) { show(resultEl, String(e), "err"); }
